...
class ConditionalGeneration_SSHHigh(DiffusionConstraint):
    ...
    def __str__(self):
        return f'ConditionalGeneration_SSHHigh - field : {self.field_path}'
...
class ConditionalGeneration_STempLow(DiffusionConstraint):
    ...
    def __str__(self):
        return f'ConditionalGeneration_STempLow - field : {self.field}'
...
class ConditionalGeneration_STempHigh(DiffusionConstraint):
    ...
    def __str__(self):
        return f'ConditionalGeneration_STempHigh - field : {self.field_path}'
...
class GradientZeroMeanConstraint(DiffusionConstraint):
    ...
    def __str__(self):
        return f'GradientZeroMeanConstraint - beta : {self.beta} dims : {self.dims}'
...
class GradientZeroMeanDensityConstraint(DiffusionConstraint):
    def __init__(self, beta, beta_type='constant', dims=[-1, -2]):
        ...
        train_dataloader = get_dataloader(config.data_file, batch_size=1,
                                                fields=config.fields, normalisation='3-std', 
                                                transform=True, shuffle=True, device=self.device)
        self.extractor = train_dataloader.get_transform().uncall
        # #mask for density metric

        #mean density of the training set
        self.mean_density = torch.tensor(np.loadtxt(f'{PRP}/Results/analysis_scripts/mean_density_train.txt'),
    ...
    def apply(self, x, t=None, n_iter=1): 
        ...
        for i in range(n_iter):
            ...
            beta = self.beta.get_beta(t) if t is not None else self.beta
            print(f'apply constraint {beta}: {loss}')
...
class MeanGradientDensityConstraint(DiffusionConstraint):
    def __init__(self, beta, beta_type='constant', dims=[-1, -2]):
        ...
        #mean density of the training set
        self.mean_density = torch.tensor(np.loadtxt(f'{PRP}/Results/analysis_scripts/mean_density_train.txt'), device='mps', dtype=torch.float32)
    ...
    def apply(self, x, t=None, n_iter=1): 
        ...
        for i in range(n_iter):
            ...
            with torch.enable_grad():
                ...
                grad_mean_density[:, -1].fill_(0.0)

                #compute density vertical gradient
                density_profile = density.nanmean(dim=self.dims)
                grad_density = (density_profile[:,:-1] - density_profile[:,1:])
                grad_density[:, -1].fill_(0.0)

                diff = (grad_mean_density - grad_density)**2
                loss = torch.nansum(diff)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
            ...
            beta = self.beta.get_beta(t) if t is not None else self.beta
            print(f'apply constraint c {beta}: {loss.item()}')
...
class NegativeGradientDensityConstraint(DiffusionConstraint):
    def __init__(self, beta, beta_type='constant', dims=[-1, -2]):
        super().__init__()
        self.beta = Beta(beta=beta, beta_type=beta_type)
        self.dims = dims
    ...
    def apply(self, x, t=None, n_iter=1): 
        ...
        for i in range(n_iter):
            ...
            with torch.enable_grad():
                ...
                loss = torch.nansum(grad_density[grad_density > 0.0])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
            ...
            beta = self.beta.get_beta(t) if t is not None else self.beta
            print(f'apply constraint b {beta}: {loss.item()}')

            x -= beta * grad

        self.beta.update(esp=loss, eta=0.0001)
        return x.detach()
    

def NegativeGradientDensityOptimisatione(DiffusionConstraint) : 
    def __init__(self):
