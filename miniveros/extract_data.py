...
# ----------------------------------------------------------------------------- split
def choose_holdout(run_ck: np.ndarray, run_eps: np.ndarray, mode: str, n: int, seed: int) -> np.ndarray:
...
def main(argv=None):
    ...
    p.add_argument("--holdout-mode", default="interior_random", choices=["interior_random", "row_ck_max", "none"])
    ...
    water = ~land_mask
    holdout = choose_holdout(run_ck, run_eps, args.holdout_mode, args.n_holdout, args.seed)
    ...
    meta = dict(raw_dir=os.path.abspath(args.raw_dir), fields=fields, last_years=args.last_years, stride=args.stride,
                n_runs=n_run, n_keep_per_run=n_keep, holdout_mode=args.holdout_mode, seed=args.seed,
