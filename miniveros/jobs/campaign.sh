...
# usage:  jobs/campaign.sh [-s split_name] [-x "extract_data.py args"] [norm_mode ...]     default mode: 3-std
#   -s NAME   hold-out split name: data goes to data/veros_acc_TS_NAME.npz, runs to runs/NAME_<mode>
#   -x ARGS   extra extraction arguments, e.g. "--holdout-mode rows --holdout-ck 0.126,0.2,0.3175"
#   examples: jobs/campaign.sh -s band3 -x "--holdout-mode rows --holdout-ck 0.126,0.2,0.3175"
#             jobs/campaign.sh -s top   -x "--holdout-mode row_ck_max"
...
SPLIT=""; XARGS=""
while getopts "s:x:" opt; do case $opt in s) SPLIT=$OPTARG ;; x) XARGS=$OPTARG ;; *) exit 1 ;; esac; done; shift $((OPTIND - 1))
MODES=("$@"); [ ${#MODES[@]} -gt 0 ] || MODES=(3-std)
PREFIX="full_"
if [ -n "$SPLIT" ]; then export MV_DATA="${MV_DATA%.npz}_${SPLIT}.npz"; PREFIX="${SPLIT}_"; fi
echo "split: ${SPLIT:-default}  data: $MV_DATA  extraction args: ${XARGS:-none}"
...
J_EXTRACT=$(sb --account="$MV_ACCOUNT_CPU" --qos="$MV_QOS_CPU" --partition="$MV_CPU_PARTITION" \
               --output="$MV_WORK/logs/extract_${PREFIX}%j.out" --error="$MV_WORK/logs/extract_${PREFIX}%j.out" --export=ALL "$HERE/extract.sbatch")
