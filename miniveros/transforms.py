...
Forward  (``normalise``):   {field: (..., Z, Y, X)}  ->  concat channels -> normalise -> zero land -> zero-pad
...
Land and padding cells are 0 in network space (the level mean); at sampling they are re-imposed after every step,
at the noise level of the step by default (see pipeline.LandZero).
"""
...
class FieldTransform:
    ...
    def __init__(self, field_levels: dict[str, int], mean: torch.Tensor, std: torch.Tensor,
        ...
        # cells that are exactly zero in network space: land + padding
